...
type ImageSource = string | { uri: string } | number | React.ComponentType<{ width?: number | string; height?: number | string }>;
...
function getHeadingText(heading: string | { line1: string; line2?: string; line3?: string }): string {
    if (typeof heading === "string") return heading;
    return [heading.line1, heading.line2, heading.line3].filter(Boolean).join(" ");
}

// Helper to get heading lines as array
function getHeadingLines(heading: string | { line1: string; line2?: string; line3?: string }): string[] {
    if (typeof heading === "string") return [heading];
    return [heading.line1, heading.line2, heading.line3].filter((line): line is string => Boolean(line));
}

// Helper to check if image is an SVG component
function isSvgComponent(image: ImageSource): image is React.ComponentType<{ width?: number | string; height?: number | string }> {
...
function getSvgComponent(image: ImageSource): React.ComponentType<{ width?: number | string; height?: number | string }> | null {
    if (typeof image === 'object' && image !== null && 'default' in image) {
        return (image as { default: React.ComponentType<{ width?: number | string; height?: number | string }> }).default;
    }
    if (typeof image === 'function') {
        return image as React.ComponentType<{ width?: number | string; height?: number | string }>;
    }
    return null;
}
...
function HeroHeading({ heading, animationVariant, animationDelay, multiLine = false }: HeroHeadingProps) {
